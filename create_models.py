...
def train_models(df):
    ...
    print("Training nate_decision_tree...")
    dt_model.fit(X_train, y_train)
    
    # Evaluate
    y_pred = dt_model.predict(X_test)
    accuracy = accuracy_score(y_test, y_pred)
    print(f"nate_decision_tree accuracy: {accuracy:.3f}")
    
    # Save model
    model_path = 'models/nate_decision_tree.sav'
    joblib.dump(dt_model, model_path)
    print(f"Saved nate_decision_tree to {model_path}")
